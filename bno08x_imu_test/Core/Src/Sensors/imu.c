...
bool BNO08x_Init(BNO08x_Handle_t *dev, I2C_HandleTypeDef *hi2c)
{
    ...
    // (Optional) you could send an "executable on" command on channel 1 here.
    // For many breakouts, just waiting is enough.
...
bool BNO08x_ReadGyro(BNO08x_Handle_t *dev, float *gx, float *gy, float *gz)
{
    uint8_t channel;
    uint16_t len;
    ...
    if (channel != BNO08X_CH_REPORTS) {
        // Not a sensor report, ignore
        return false;
    }
    ...
    int16_t gx_raw = (int16_t)((uint16_t)p[4] | ((uint16_t)p[5] << 8));
    int16_t gy_raw = (int16_t)((uint16_t)p[6] | ((uint16_t)p[7] << 8));
    int16_t gz_raw = (int16_t)((uint16_t)p[8] | ((uint16_t)p[9] << 8));

    *gx = gyroRawToDps(gx_raw);
    *gy = gyroRawToDps(gy_raw);
