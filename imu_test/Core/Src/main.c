...
uint8_t MPU6050_Init(void) {
    ...
    for (int i = 0; i < 2; i++) {
        ...
        if (HAL_I2C_IsDeviceReady(&hi2c1, MPU6050_ADDR, 2, 100) == HAL_OK) {
            ...
            if (check == 0x68 || check == 0x6B) {  // Valid WHO_AM_I values
                found = 1;
                break;
            }
        }
...
int main(void)
{
  ...
  for (uint8_t i = 1; i < 128; i++) {
      HAL_StatusTypeDef result = HAL_I2C_IsDeviceReady(&hi2c1, (i << 1), 1, 100);
      if (result == HAL_OK) {
  ...
  if (device_count == 0) {
      ...
      sprintf(uart_buffer, "\r\nTroubleshooting:\r\n");
      HAL_UART_Transmit(&huart1, (uint8_t*)uart_buffer, strlen(uart_buffer), HAL_MAX_DELAY);
      sprintf(uart_buffer, "1. Check wiring: VCC→3.3V, GND→GND, SCL→PB6, SDA→PB7\r\n");
      HAL_UART_Transmit(&huart1, (uint8_t*)uart_buffer, strlen(uart_buffer), HAL_MAX_DELAY);
      sprintf(uart_buffer, "2. Verify pull-up resistors on SDA/SCL (4.7k to 3.3V)\r\n");
      HAL_UART_Transmit(&huart1, (uint8_t*)uart_buffer, strlen(uart_buffer), HAL_MAX_DELAY);
      sprintf(uart_buffer, "3. Check CubeMX: I2C1 enabled, PB6=SCL, PB7=SDA\r\n");
      HAL_UART_Transmit(&huart1, (uint8_t*)uart_buffer, strlen(uart_buffer), HAL_MAX_DELAY);
...
static void MX_I2C1_Init(void)
{
  ...
  hi2c1.Instance = I2C1;
  hi2c1.Init.ClockSpeed = 50000;  // Reduced to 50kHz for better compatibility
